...
/*
  removeSetAndList: function() {
    const db = wx.cloud.database();
    db.collection("set").doc(this.data.setID).remove({
      success: function(res) {
        console.log("Remove set succeeded");
        this.removeListBySetID();
      },
      fail: function(err) {
        console.log("Error: Remove set failed");
        console.log(err);
      }
    })
  },

  removeListBySetID: function() {
    const db = wx.cloud.database();
    db.collection("list").where({
      set_id: this.data.setID
    }).remove({
      success: function(res) {
